...
// Login 用户登录
func (s *UserService) Login(username, password string) (*usermodels.User, string, error) {
...
// Create 创建用户
func (s *UserService) Create(user *usermodels.User) error {
	// 密码加密
...
// Update 更新用户
func (s *UserService) Update(id uint, user *usermodels.User) error {
	var existUser usermodels.User
...
func (s *UserService) UpdateByUsername(username string, user *usermodels.User) error {
	var existUser usermodels.User
...
func (s *UserService) Delete(id uint) error {
	return database.Db.Delete(&usermodels.User{}, id).Error
}

// DeleteByUsername 根据用户名删除用户
func (s *UserService) DeleteByUsername(username string) error {
	return database.Db.Where("username = ?", username).Delete(&usermodels.User{}).Error
}

// GetByID 根据ID获取用户
func (s *UserService) GetByID(id uint) (*usermodels.User, error) {
	var user usermodels.User
...
func (s *UserService) GetByUsername(username string) (*usermodels.User, error) {
	var user usermodels.User
...
// GetList 获取用户列表
func (s *UserService) GetList(page, pageSize int, username, phone, status string) ([]usermodels.User, int64, error) {
	var users []usermodels.User
	var total int64

	query := database.Db.Model(&usermodels.User{})
